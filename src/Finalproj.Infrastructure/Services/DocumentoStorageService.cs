...
/// Aplica limite de tamanho, extensão, validação de conteúdo (magic bytes) e cifragem opcional em repouso.
/// </summary>
public class DocumentoStorageService : IDocumentoStorageService
{
    ...
    /// <inheritdoc />
    public Task ValidarFicheiroParaUploadAsync(UploadedFileContent ficheiro, CancellationToken cancellationToken = default) =>
        _contentValidator.ValidarAsync(ficheiro, cancellationToken);

    /// <inheritdoc />
    public async Task<string> GuardarFicheiroAsync(string pastaRelativaBase, int entidadeId, UploadedFileContent ficheiro, string prefixoNome, CancellationToken cancellationToken = default)
    {
        ...
        await _cifragem.EscreverBytesAsync(caminhoFisico, ficheiro.Content, cancellationToken);
    ...
    /// <inheritdoc />
    public async Task<string> GuardarFicheiroNoCaminhoRelativoAsync(string caminhoRelativo, UploadedFileContent ficheiro, CancellationToken cancellationToken = default)
    {
        await ValidarAntesDeGuardarAsync(ficheiro, cancellationToken);
    ...
    /// <inheritdoc />
    public async Task<string> GuardarBytesNoCaminhoRelativoAsync(string caminhoRelativo, byte[] conteudo, CancellationToken cancellationToken = default)
    {
        var normalizado = caminhoRelativo.Replace('\\', '/').TrimStart('/');
        var nomeFicheiro = Path.GetFileName(normalizado);
        if (string.IsNullOrEmpty(nomeFicheiro))
        ...
        var caminhoFisico = Path.Combine(pastaFull, nomeFicheiro);
        await _cifragem.EscreverBytesAsync(caminhoFisico, conteudo, cancellationToken);
        return normalizado;
    }

    /// <inheritdoc />
    public async Task<byte[]?> LerConteudoAsync(string? caminhoRelativo, CancellationToken cancellationToken = default)
    {
        var caminhoFisico = ResolverCaminhoFisicoParaLeitura(caminhoRelativo);
        if (string.IsNullOrEmpty(caminhoFisico) || !File.Exists(caminhoFisico))
            return null;
        return await _cifragem.LerBytesAsync(caminhoFisico, cancellationToken);
    }
    ...
    private async Task ValidarAntesDeGuardarAsync(UploadedFileContent ficheiro, CancellationToken cancellationToken)
    {
